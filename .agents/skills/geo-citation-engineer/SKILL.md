...
- Default (no extra flag) — DuckDuckGo HTML, stdlib, no token. If the HTML endpoint is blocked, search Wikipedia for the product topic. If that also fails, keep the requested query with an empty SERP (do not load the CRM fixture).
